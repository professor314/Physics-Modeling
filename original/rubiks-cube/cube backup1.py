...
imaginary_positions=[]
for n in range(0,53):
    imaginary_positions.append(imaginary_list[n].pos)
print(imaginary_positions)  # Python 3 fix
#positions:
#side 1:
pa1= [0,2,4]
pb1= [1,2,4]    
...
def cross1():
    a = cubelet_search(h1)
    print(cubelet_search(h1))  # Python 3 fix
    if a[2] == 1:
        while x.pos != id4.pos:
            clock(back)
    counter(left)
...
def solve():
    ...
    m=0
    for n in range(0,53):
        while imaginary_positions[n].x != facelets[n].pos.x and imaginary_positions[n].y != facelets[m].pos.y and imaginary_positions[n].z != facelets[m].pos.z:
            mix(1)
            count = count+1
            if count == 1000000:
                count_million +=1
                count=0
    print("I did it in",count_million,"million",count,"moves!")  # Python 3 fix
##while 1:

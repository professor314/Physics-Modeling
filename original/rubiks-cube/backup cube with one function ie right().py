...
def right():
    ...
    faceletslistz = []
    sizes = []
    faceletsmove = [] #the facelets that will move
    while m<=53 and n<=40:
...
def left():
    m=0
    n=0
    if debug is true:print("initial",m,n)  # Python 3 fix
    positions1 = (pa1,pa6,pd1,pd6,pg1,pg6,pa5,pa1,pd5,pd1,pg5,pg1,pc3,pg5,pf3,pd5,pi3,pg5,pa6,pi3,pd6,pf3,pg6,pi3,pa4,pc4,pb4,pf4,pc4,pi4,pd4,pb4,pf4,ph4,pg4,pa4,ph4,pd4,pi4,pg4)
    faceletslistx = []
    faceletslisty = []
    faceletslistz = []
    sizes = []
    ...
    while m<=53 and n<=40:
        if (positions1[n])[0] == facelets[m].pos.x and (positions1[n])[1] == facelets[m].pos.y and (positions1[n])[2] == facelets[m].pos.z:
            #if debug is true:print "x",facelets[m].pos.x , (positions1[n+1])[0]
            #if debug is true:print 'y',facelets[m].pos.y , (positions1[n+1])[1]
            #if debug is true:print 'z',facelets[m].pos.z , (positions1[n+1])[2]
            faceletsmove.append(facelets[m])
            faceletslistx.append((positions1[n+1])[0])
            faceletslisty.append((positions1[n+1])[1])
            faceletslistz.append((positions1[n+1])[2])
    ##            facelets[m].size.x = (sizes[n+1])[0]         
    ##            facelets[m].size.y = (sizes[n+1])[1]
    ##            facelets[m].size.y = (sizes[n+1])[2]
            #print faceletslistx,faceletslisty,faceletslistz
            if m != 53:
                m = m+1
                print("not yet")  # Python 3 fix
            elif m == 53:
                m = 0
                n = n+2
                print(not yet)  # Python 3 fix
                ...
                m = m+1
                if debug is true: print("not yet",m,n)  # Python 3 fix
        elif m == 53:
                m = 0
                n = n+2
                print("not yet")  # Python 3 fix
    #print "x = ",faceletslistx
    #print "y = ",faceletslisty
    #print "z = ",faceletslistz
    n=0
    print(faceletslistx[n],positions1[n])  # Python 3 fix
    while n<21:
        faceletsmove[n].pos.x = faceletslistx[n]
        faceletsmove[n].pos.y = faceletslisty[n]
        faceletsmove[n].pos.z = faceletslistz[n]
        #faceletsmove[n].rotate(angle = pi/2)
        #faceletsmove[n].size = faceletsmove[n+3]
    ##        faceletsmove[n].size 
    ##        faceletsmove[n].size 
        n=n+1
while True:

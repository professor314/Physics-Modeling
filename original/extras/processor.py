...
class ALU:
    ...
    def evaluate(self):
        ...
        if self.output==0:
            self.zero = 1
            if DEBUG: print("ALU: setting zero flag")  # Python 3 fix
        else:
            if DEBUG and self.zero==1: print("ALU: clearing zero flag")  # Python 3 fix
            self.zero = 0

        #set the sign flag
        if self.output<0:
            self.sign = 1
            if DEBUG: print("ALU: setting sign flag")  # Python 3 fix
        else:
            if DEBUG and self.sign==1: print("ALU: clearing sign flag")  # Python 3 fix
            self.sign = 0
...
class Sequencer:
    ...
    def fetch(self):
        if LOGINSTRUCTIONS: print("sequencer: Program Counter = -----> %2d <------" %self.program_counter)  # Python 3 fix
    ...
    def decode(self):
        ...
        if LOGDECODE: print("sequencer: decode",self.instruction,"->",self.opcode,self.src1,self.src2,self.dest)  # Python 3 fix
    ...
    def doload(self):
        ...
        if LOGINSTRUCTIONS: print("sequencer: loading",self.memory.data,"to reg"+str(self.dest))  # Python 3 fix
    ...
    def dostore(self):
        ...
        if LOGINSTRUCTIONS: print("sequencer: storing", self.regfile.read(self.src1),  # Python 3 fix
                                  "at address",self.memory.address)
        self.memory.data = self.regfile.read(self.src1)
        self.memory.write()

    def domove(self):
        if LOGINSTRUCTIONS: print("sequencer: moving r"+str(self.src1)+" to r"+str(self.dest))  # Python 3 fix
    ...
    def doadd(self):
        ...
        if LOGINSTRUCTIONS: print("sequencer: adding",self.alu.A,"+",self.alu.B)  # Python 3 fix
        self.alu.opcode = ADD
        self.alu.evaluate()
    ...
    def dosubtract(self):
        self.alu.A = self.regfile.read(self.src1)
        self.alu.B = self.regfile.read(self.src2)
        if LOGINSTRUCTIONS: print("sequencer: subtracting",self.alu.A,"-",self.alu.B)  # Python 3 fix
        self.alu.opcode = SUB
        self.alu.evaluate()
        self.regfile.write(self.dest,self.alu.output)

    def dodivide(self):
        if LOGINSTRUCTIONS: print("sequencer: dividing")  # Python 3 fix
        self.alu.A = self.regfile.read(self.src1)
        self.alu.B = self.regfile.read(self.src2)
        self.alu.opcode = DIV
        self.alu.evaluate()
    ...
    def doincrement(self):
        if LOGINSTRUCTIONS: print("sequencer: incrementing r"+str(self.dest))  # Python 3 fix
    ...
    def domultiply(self):
        if LOGINSTRUCTIONS: print("sequencer: multiplying")  # Python 3 fix
        self.alu.A = self.regfile.read(self.src1)
        self.alu.B = self.regfile.read(self.src2)
        self.alu.opcode = MUL
        self.alu.evaluate()
    ...
    def dojump(self):
        if LOGINSTRUCTIONS: print("sequencer: jumping to",self.dest)  # Python 3 fix
        self.program_counter=self.dest

    def dojumpifzero(self):
        if self.alu.zero:
            if LOGINSTRUCTIONS: print("sequencer: jump on zero to",self.dest)  # Python 3 fix
            self.program_counter=self.dest
        else: 
            if LOGINSTRUCTIONS: print("sequencer: jump if zero failed")  # Python 3 fix
            
    def dojumpifnegative(self):
        if self.alu.sign:
            if LOGINSTRUCTIONS: print("sequencer: jump if negative to",self.dest)  # Python 3 fix
            self.program_counter=self.dest
        else: 
            if LOGINSTRUCTIONS: print("sequencer: jump if negative failed")  # Python 3 fix
            
    def dohalt(self):
        if LOGINSTRUCTIONS: print("sequencer: halting")  # Python 3 fix
        self.status='halted'
...
class RegisterFile:
    ...
    def read(self,index):
        if LOGREGISTERS: print("regfile: read reg"+str(index)+"=="+str(self.register[index]))  # Python 3 fix
        if index<0 or index >=self.size:
            print("register file: READ ERROR, invalid register number",index,"*************")  # Python 3 fix
        return self.register[index]

    def write(self,index,value):
        if LOGREGISTERS: print("regfile: wrote reg"+str(index)+"="+str(value))  # Python 3 fix
        if index<0 or index >=self.size:
            print("register file: WRITE ERROR, invalid register number",index,"*************")  # Python 3 fix
        self.register[index] = value

    def dump(self):
        for i in range(len(self.register)):
            print("%2d %5d" % (i,self.register[i]))  # Python 3 fix
...
class Memory:
    ...
    def dump(self):  # Python 3 fix
        width = 8

        #print header (column addresses)
        print("    ", end=" ")  # Python 3 fix
        for col in range(width):
            print("%5d" % col, end=" ")  # Python 3 fix
        print()  # Python 3 fix

        print('-'*55)  # Python 3 fix

        address=0
        for row in range(width):
            print("%02d |" % int(row*width), end=" ")  # Python 3 fix
            for col in range(width):
                print("%5d" % self.word[address], end=" ")  # Python 3 fix
                address += 1
            print()  # Python 3 fix

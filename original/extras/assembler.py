...
def assemble(instruction):
    ...
    inst=field[1]
    if DEBUG: print("assembling",instruction)  # Python 3 fix
    ...
    if inst=="DATA":
        ...
        print("\nassembler: ERROR - Unrecognized instruction",inst)  # Python 3 fix
        sys.exit(0)
...
def load_program(filename,memory):
  print("Loading program from '"+filename+"'....", end=" ")  # Python 3 fix
  program = open(filename,'r')
  address=0
  for line in program:
      # ignore blank lines and comments
      if line[0] != '#' and line[0] != ' ' and line[0] != '\n': 
          if DEBUG: print("instruction:",line)  # Python 3 fix
          address,instruction = assemble(line)
          if DEBUG: print("load",address,"with",instruction)  # Python 3 fix
          memory.word[address]=instruction # load instruction into memory
  print("Done.")  # Python 3 fix

...
##right_down = button(pos = (0,0),width = 60, length =100, text = "r-d", action = lambda: counter(right))
##right_up = button(pos = (60,0),width = 60, length =100, text = "r-u", action = lambda: clock(right))
##left_down = button(pos = (0,40),width = 60, length =100, text = "l-d", action = lambda: clock(left))
##left_up = button(pos = (60,40),width = 60, length =100, text = "l-u", action = lambda: counter(left))
##bottom_left = button(pos = (120,40),width = 60, length =100, text = "b-l", action = lambda: counter(left))
##bottom_right = button(pos = (120,40),width = 60, length =100, text = "b-r", action = lambda: clock(left))
##scramble = button(pos = (60,40),width = 60, length =100, text = "mix", action = lambda: mix(50))
#side 1
...
def clock(x): #turn face 'x' clockwise
    if x ==top: face = top  #these are so the function knows what it is doing(used later in if statements)
    ...
    n=0
    while n<21:
        faceletsmove[n].pos.x = faceletslistx[n] #these will change the x position of the facelets that move
        faceletsmove[n].pos.y = faceletslisty[n] #these will change the y position of the facelets that move
        faceletsmove[n].pos.z = faceletslistz[n] #these will change the z position of the facelets that move
        if face == front:
            ...
            if faceletsmove[n].size == ia6.size:
                ...
                faceletsmove[n].size = ia5.size
        n=n+1
def counter(x):#for counter-clockwise moves of face 'x'(goes so fast you can't tell)
    clock(x)
    clock(x)
    clock(x)
def mix(x): #will mix up the cube by making x random moves
    k = 0
    while k<=x:
        a = random()
        if a <=.1666666:
        ...
        k =1+k
##while True:
##    c.interact() # will check for mouse events and drive specified actions
while True:
    input()

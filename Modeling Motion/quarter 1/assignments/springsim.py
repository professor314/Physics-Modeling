...
curve=curve(radius=.01,color=color.blue)

#make a rod for the pendulum to appear to hang from
Rod = cylinder(radius=0.05,length=2,axis=(0,0,-1))
...
pend = SpringPendulum()
pend.theta = 7*pi/8            # angle relative to vertical
pend.velocity = vector(0,0,.5) # give an initial push in the z direction
pend.length=2.0                # length pendulum is extended out to
pend.spring.rest_length=2.0    # length the spring tries to be
pend.spring.damping=.05        # how much friction dampens the motion
pend.spring.k=10.0
...
t=0
dt=1.0/32 #time intervals between animation frames
